...
export function useFeatureAccess() {
  ...
  const getRemainingSearches = () => {
    ...
    return Math.max(0, currentFeatures.monthlySearches - (user?.searchCount || 0));
  };
  
  // Get search usage percentage
  const getUsagePercentage = () => {
    if (currentFeatures.monthlySearches === -1) return 0;
    return Math.min(100, ((user?.searchCount || 0) / currentFeatures.monthlySearches) * 100);
  };
  ...
  const getUpgradeMessage = (feature: keyof FeatureLimits) => {
    const messages = {
      exportToPDF: "Upgrade to Scholar or higher to export search results to PDF",
      priorityProcessing: "Upgrade to Scholar or higher for priority search processing",
      advancedAnalytics: "Upgrade to Scholar or higher for advanced analytics and insights",
      ...
      teamCollaboration: "Upgrade to Institution for team collaboration features",
      whiteLabel: "Upgrade to Institution for white-label options",
      dedicatedSupport: "Upgrade to Professional or higher for dedicated support",
      monthlySearches: "You've reached your monthly search limit. Upgrade for more searches.",
    };
