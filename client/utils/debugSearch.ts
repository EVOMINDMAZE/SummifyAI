...
export async function debugSearchIssue(query: string) {
  ...
  try {
    ...
    debugInfo.steps.push(`✅ Books table accessible (sample: ${booksTest[0]?.title})`);
    ...
    debugInfo.steps.push(`✅ Chapters table accessible (sample: ${chaptersTest[0]?.chapter_title})`);
    ...
    debugInfo.steps.push(`✅ Search query successful: ${searchResults?.length || 0} results`);
    debugInfo.results = {
