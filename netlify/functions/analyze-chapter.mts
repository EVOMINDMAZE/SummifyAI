...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    if (!chapter || !query) {
      return new Response(
        JSON.stringify({ error: "Chapter data and query are required" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
    ...
    const grokApiKey = Netlify.env.get("GROK_API_KEY");
    if (!grokApiKey) {
      return new Response(
    ...
    console.log(
      `📄 Netlify Function: Analyzing chapter "${chapter.chapter_title}" for query "${query}"`,
    );
      ...
      relevanceScore: Math.min(
        ...
        Math.max(
          ...
          aiResult.relevanceScore ||
            Math.round((1 - (chapter.similarity_score || 0.5)) * 100),
        ),
      ),
      whyRelevant:
