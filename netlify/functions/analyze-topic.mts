        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert search analyst for business books. Always respond with valid JSON only.",
          },
    ...
    return new Response(
      JSON.stringify({
        ...
        data: {
          ...
          explanation:
            result.explanation ||
            `Analysis of "${topic}" completed successfully.`,
          refinements: Array.isArray(result.refinements)
            ? result.refinements.slice(0, 3)
            : [],
        },
      }),
      {
