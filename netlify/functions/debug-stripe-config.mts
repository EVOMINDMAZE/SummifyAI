...
export const handler: Handler = async (event) => {
  ...
  const config = {
    ...
    priceIds: {
      ...
      professionalMonthly: process.env.STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID || 'NOT_SET',
      professionalAnnual: process.env.STRIPE_PROFESSIONAL_ANNUAL_PRICE_ID || 'NOT_SET',
      institutionMonthly: process.env.STRIPE_INSTITUTION_MONTHLY_PRICE_ID || 'NOT_SET',
      institutionAnnual: process.env.STRIPE_INSTITUTION_ANNUAL_PRICE_ID || 'NOT_SET',
    },
    fallbackIds: {
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      instructions: {
        ...
        step4: 'Price IDs should start with "price_" followed by random characters',
