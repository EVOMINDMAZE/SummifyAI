...
const getPlanFromPriceId = (priceId: string) => {
  const priceMapping: Record<string, { plan: string; limit: number }> = {
    // Scholar Plan
    [process.env.STRIPE_SCHOLAR_MONTHLY_PRICE_ID!]: { plan: 'scholar', limit: 500 },
    [process.env.STRIPE_SCHOLAR_ANNUAL_PRICE_ID!]: { plan: 'scholar', limit: 500 },
    
    // Professional Plan
    [process.env.STRIPE_PROFESSIONAL_MONTHLY_PRICE_ID!]: { plan: 'professional', limit: 2000 },
    [process.env.STRIPE_PROFESSIONAL_ANNUAL_PRICE_ID!]: { plan: 'professional', limit: 2000 },
    
    // Institution Plan
    [process.env.STRIPE_INSTITUTION_MONTHLY_PRICE_ID!]: { plan: 'institution', limit: 999999 },
    [process.env.STRIPE_INSTITUTION_ANNUAL_PRICE_ID!]: { plan: 'institution', limit: 999999 },
  };
...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  if (error) {
    ...
    console.log(`Updated user ${customerEmail} with Stripe customer ID ${customerId}`);
  }
}
...
async function handleSubscriptionUpdate(subscription: Stripe.Subscription) {
  ...
  if (error) {
    ...
    console.log(`Updated subscription for customer ${customerId} to ${planDetails.plan} plan`);
  }
}
