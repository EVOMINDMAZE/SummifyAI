...
async function checkTableExistence() {
  ...
  for (const tableName of expectedTables) {
    try {
      ...
      console.log(
        `${results[tableName].exists ? "✅" : "❌"} ${tableName}: ${results[tableName].exists ? `${count || 0} rows` : results[tableName].error}`,
      );
    } catch (error) {
      results[tableName] = { exists: false, error: error.message };
...
async function testSearchFunctionality() {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .or(
        `chapter_title.ilike.%${searchQuery}%,chapter_text.ilike.%${searchQuery}%,books.title.ilike.%${searchQuery}%`,
      )
...
async function generateReport() {
  ...
  if (issues.length > 0) {
    ...
    if (!searchOk) {
      ...
      console.log(
        "• Verify foreign key relationship between books and chapters",
      );
    }
  } else {
